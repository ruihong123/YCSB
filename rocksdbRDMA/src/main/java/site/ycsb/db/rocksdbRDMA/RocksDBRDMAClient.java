...
package site.ycsb.db.rocksdbRDMA;
 ...
 */
public class RocksDBRDMAClient extends DB {
  ...
  private static final Logger LOGGER = LoggerFactory.getLogger(RocksDBRDMAClient.class);

  @GuardedBy("RocksDBRDMAClient.class") private static Path rocksDbDir = null;
  @GuardedBy("RocksDBRDMAClient.class") private static Path optionsFile = null;
  @GuardedBy("RocksDBRDMAClient.class") private static RocksObject dbOptions = null;
  @GuardedBy("RocksDBRDMAClient.class") private static RocksDB rocksDb = null;
  @GuardedBy("RocksDBRDMAClient.class") private static int references = 0;
  ...
  public void init() throws DBException {
    synchronized(RocksDBRDMAClient.class) {
      if(rocksDb == null) {
   ...
   *
   * Should only be called with a {@code synchronized(RocksDBRDMAClient.class)` block}.
   *
  ...
  private RocksDB initRocksDBWithOptionsFile() throws IOException, RocksDBException {
    ...
    System.out.print(cfDescriptors.get(0).getName());
